 ...
 * 	Allow Alignment of table when No reconstruction asked
 * 	
...
//if (Choice_macro == "Reconstruction" || Choice_macro == "Registration+Reconstruction") { Version = 4.0;
	...
	for (u = 0; u < umax; u++) {
		...
		// Collect data in arrays
			...
			VFB=newArray(N);
			VC=newArray(N);
			vb_count = 0; Xvb=0; Yvb=0; // actually the X and Y of scan starts at (1, 1) in the main CSV file.
			fvb_count = 0; Xfvb=0; Yfvb=0;
			vc_count = 0; Xvc=0; Yvc=0;
			...
			if(STDDEV_exists == true){
				...
				VSTD=newArray(N);
				VPMIN=newArray(N);
				std_count = 0; Xstd=0; Ystd=0;
				pmin_count = 0; Xpmin=0; Ypmin=0;
			}
			if (PRO_exists == true) {
				PRO_all=newArray(Table_size);
				VPRO=newArray(N);
				vpro_count = 0; Xpro=0; Ypro=0;
			}
				...
				for (i = 0; i < Table_size; i++) {
					...
					if(Bool_virtual_brightfield == 1) { //Full Virtual Brightfield :each pixel is the of some all spots intensities
						if (X_scan[i] != Xvb || Y_scan[i] != Yvb) {vb_count= Nx*(Y_scan[i]-1)+X_scan[i]-1; VB[vb_count]=M_all[i]; Xvb = X_scan[i];Yvb = Y_scan[i]; }
					}
					if(Bool_full_virtual_brightfield == 1) { //Full Virtual Brightfield :each pixel is the of some all spots intensities
						if (X_scan[i] != Xfvb || Y_scan[i] != Yfvb) {fvb_count= Nx*(Y_scan[i]-1)+X_scan[i]-1; VFB[fvb_count]=0; Xfvb = X_scan[i];Yfvb = Y_scan[i]; }
						VFB[fvb_count] = VFB[fvb_count]+M_all[i];					
					}
					if(Bool_spot_count_map == 1) { //spot count map :each pixel is the number of spots
						if (X_scan[i] != Xvc || Y_scan[i] != Yvc) {
							vc_count = Nx*(Y_scan[i]-1)+X_scan[i]-1; // in case no reflection, jump the gap with vc_count		
							VC[vc_count]=0; Xvc = X_scan[i];Yvc = Y_scan[i]; 
						}
					...
					if(STDDEV_exists == true){
						...
						if(Bool_StdDev_map == 1) { //spot count map :each pixel is the number of spots
							if (X_scan[i] != Xstd || Y_scan[i] != Ystd) {std_count= Nx*(Y_scan[i]-1)+X_scan[i]-1; VSTD[std_count]=STDDEV_all[i]; Xstd = X_scan[i];Ystd = Y_scan[i]; }
						}
						if(Bool_Prominence_min_map == 1) { //spot count map :each pixel is the number of spots
							if (X_scan[i] != Xpmin || Y_scan[i] != Ypmin) {pmin_count= Nx*(Y_scan[i]-1)+X_scan[i]-1; VPMIN[pmin_count]=PMIN_all[i]; Xpmin = X_scan[i];Ypmin = Y_scan[i]; }		
						}
					}
					if(PRO_exists == true){
						PRO_all[i] = Table.get('Prominence_pseudo', i);
						if(Bool_pro_pseudo_map == 1) { //spot count map :each pixel is the number of spots
							if (X_scan[i] != Xpro || Y_scan[i] != Ypro) {vpro_count= Nx*(Y_scan[i]-1)+X_scan[i]-1; VPRO[vpro_count]=0; Xpro = X_scan[i];Ypro = Y_scan[i]; }
		...
		if(Bool_no_reconstruction==1 && Bool_align_from_CSV == 1){    //Align table even if no reconstruction asked
			for (i = 0; i < Table_size; i++) {
				main_spot = false;
				//R = R_all[i];
